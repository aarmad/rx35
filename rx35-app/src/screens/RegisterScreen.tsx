...
export default function RegisterScreen() {
  ...
  const submit = async () => {
    setError(null);
    if (!nom.trim() || !telephone.trim() || password.length < 6) {
      setError("Nom et numéro requis, mot de passe de 6 caractères minimum.");
      return;
    }
    setLoading(true);
    try {
      await register(nom.trim(), telephone.trim(), password);
    } catch (err: any) {
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1 }}>
        <View style={styles.content}>
          ...
          <Pressable onPress={submit} disabled={loading} style={[styles.primaryButton, { backgroundColor: colors.primary, opacity: loading ? 0.6 : 1 }]}>
            {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.primaryButtonText}>Créer mon compte</Text>}
          </Pressable>
          ...
          <Pressable onPress={() => navigation.navigate("Login")} style={{ marginTop: spacing.lg, alignItems: "center" }}>
            <Text style={{ color: colors.textMuted, fontFamily: typography.body, fontSize: 13 }}>
              Déjà un compte ? <Text style={{ color: colors.accent, fontFamily: typography.bodySemiBold }}>Se connecter</Text>
            </Text>
          </Pressable>
        </View>
